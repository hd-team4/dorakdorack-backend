...
public class DosirakPromptGenerator {
  ...
  public DosirakGenerationResultDto generateDosirakText(List<String> answers) {
    ...
    String prompt = """
        ...
        1. 이 선호에 따라 어울릴 수 있는 한식, 양식, 일식, 중식 중 **'풍성한 메인 메뉴 중심'**의 도시락을 상상해서 메뉴를 구성해주세요.
           - 메인 메뉴는 사용자 도시락 선호 기준으로 두 가지이며, 중량감 있고 시각적으로도 **가득 차 보이게** 구성합니다.
           - ※ 주의 메인 메뉴는 사용자 선호 답변에 대한 메뉴입니다. 다른 메뉴가 들어가면 안됩니다. 사용자 선호가 고기면 해산물을 제외한 고기로 메인 메뉴를 구성해야 합니다. 만약 해산물이면 해산물에 대해서만 메인 메뉴를 구성해주세요.
           - ※ 주의 반찬은 반드시 **1~2개 이하**로만 구성합니다.
           - ⚠️ 반찬의 수가 메인 메뉴보다 많거나 도시락 내 면적에서 더 큰 비중을 차지하면 안 됩니다.
           - 반찬은 **작고 소량**이어야 하며, 메인 메뉴의 색감과 풍미를 **보완하는 역할**만 해야 합니다.
           - 메인 메뉴보다 반찬의 수량이 많거나, 도시락에서 차지하는 공간이 더 넓으면 절대 안 됩니다.
           - 메인 메뉴는 도시락의 **전체 면적 기준 70~80%% 이상을 차지해야 하며**, 반찬은 20%% 이하 공간에 소량으로 배치되어야 합니다.
           - 사용자의 도시락 선호 답변에 "현재 사용자가 가지고 있는 알러지"가 포함되어 있으면 알러지를 고려한 메뉴를 구성해주셔야 합니다.
           - 반찬은 **작고 소량**이어야 하며, **메인 메뉴의 색감과 풍미를 보완하는 역할**만 수행합니다.
           - 밥은 무조건 흰쌀밥으로 **1개** 넣어주셔야 합니다. 무조건 하나여야 합니다.
           - 도시락 통은 무조건 검은색입니다.
        ...
        3. 카테고리는 고혈압 식단, 칼로리 식단, 스페셜 식단, 단백질 식단, 당뇨 식단, 가성비 식단 중에서 도시락에 맞는 항목을 2개 이상 작성하세요. 식단 이외에 아무것도 들어가면 안됩니다.
           ...
           - ※ 주의 categories는 반드시 혈압 식단, 칼로리 식단, 스페셜 식단, 단백질 식단, 당뇨 식단, 가성비 식단 중에서 작성해주셔야합니다. 다른 식단은 절대 나오면 안됩니다.
...

        {
          ...
          "prompt":"Look at the text and draw only a packed lunch that consists of a menu, as shown in the front. The packed lunch ratio is 6 x 4, the lunch box is only black, and the packed lunch is centrally located.
                    ...
                    The texture should be matte, with no gloss or glossy highlights, and soft, diffuse natural daylight should be applied.
                    **The visual style must be hyper-realistic, resembling an actual photo of a real lunch box.**
                    ...
                    **The lunch box must be shot from a top-down (bird's eye) angle, as if taken directly from above. It should be clearly framed and centered, ideal for use as a clean thumbnail.**
                    ...
                    The food should look like it was photographed with a regular camera during the day, without artificial lighting or unrealistic reflections.
                    The image must not contain any text, brand names, logos, or writing on or around the lunch box.
                    The background must be only a single soft solid color that complements the lunch box.
